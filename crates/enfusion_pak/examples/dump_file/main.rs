...
async fn load_pak_files<P: AsRef<Path>>(dir: P) -> color_eyre::Result<AsyncVfsPath> {
    ...
    while let Some(entry) = read_dir.next_entry().await? {
        ...
        if path.extension().map(|ext| ext == "pak").unwrap_or(false) {
            let parsed_file = wrapper::parse_pak_file(path)?;
...
async fn write_file(path: AsyncVfsPath, out_path: PathBuf) {
    ...
    async_std::io::copy(&mut reader, &mut writer).await.expect("failed to write data to output");
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let target_file =
        if args.file.ends_with('/') { args.file.trim_end_matches('/') } else { args.file.as_str() };
